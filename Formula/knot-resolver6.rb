class KnotResolver6 < Formula
  ...
  def install
    ...
    venv = virtualenv_install_with_resources without: ["prometheus-client", "watchdog", "pathtools"]
    if build.with? "prometheus"
      venv.pip_install "prometheus-client" 
    end
    if build.with? "watchdog"
      venv.pip_install "watchdog" 
      venv.pip_install "pathtools"
    end
